...
public class Main {
	...
	public static void removeDepartmentFromDatabase(int index) {
		Department de = myDatabase.getDepartment(index);
		//TODO: Remove a department at a given index from the database.
		myDatabase.removeDepartment(index);
		...
		//for a good reason.
		for (GuildMember mem : de.getAllMembers()) {
			myDatabase.getDepartment(0).addMember(mem);
		}
		//TODO: Print a message that the department is removed and that all members are moved to the unassigned department.
		System.out.println("Department removed. "+de.getAllMembers().size()+"( member(s) added to Unassigned Department");
	}
		...
		while(true) {
			try {
				...
				case 1:
					...
					if(!createSuccess) System.out.println("Failed to create department. Returning to Main Menu.");
					System.out.println("====================");
					break;
				case 2:
					removeDepartment();
	...
	public static void navigateGuildDatabase() {
		while(true) {
			...
			if(choiceNumber == myDatabase.departmentCount() ) return;
	...
	public static void removeDepartment() {
		while(true) {
			...
			else if(choiceNumber == 0) System.out.println("You cannot remove the Unassigned Department!");
		...
		while(true) {
			...
			if(choiceNumber == myDatabase.departmentCount() ) return;
				...
				while(true) {
					...
					if (memberChoiceNumber == chosenDept.memberCount()) break;
